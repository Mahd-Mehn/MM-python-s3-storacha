    ...
    def update(
        ...
        with self._lock:
            ...
            self._progress_history.append(
                (current_time, self.completed_objects, self.transferred_bytes)
            )
...
def create_console_progress_callback(show_details: bool = True) -> ProgressCallback:
    ...
    def console_callback(progress: MigrationProgress) -> None:
        ...
        if show_details:
            print(
                f"\rProgress: {progress.progress_percentage:.1f}% "
            ...
            print(
                f"\rProgress: {progress.progress_percentage:.1f}%", end="", flush=True
            )
...
def create_logging_progress_callback(
    logger_name: str = __name__, log_interval: int = 10
) -> ProgressCallback:
