...
class ErrorHandler:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None) -> None:
        ...
        self.js_error_patterns = {
            ...
            'network_error': re.compile(r'(ECONNREFUSED|ENOTFOUND|ETIMEDOUT|ECONNRESET)', re.IGNORECASE),
            'auth_error': re.compile(r'(Unauthorized|Forbidden|Invalid.*credentials)', re.IGNORECASE),
            'not_found': re.compile(r'(Not Found|404|NoSuchBucket|NoSuchKey)', re.IGNORECASE),
            'permission_error': re.compile(r'(Access Denied|Permission denied|EACCES)', re.IGNORECASE)
        }
    
    def handle_js_error(self, stderr: str, returncode: int, command: Optional[str] = None) -> JSWrapperError:
        ...
        if error_info['type'] == 'network':
            ...
            message = f"Authentication error in JavaScript wrapper: {error_info['message']}"
        elif error_info['type'] == 'not_found':
            message = f"Resource not found in JavaScript wrapper: {error_info['message']}"
    ...
    def handle_network_error(self, error: Exception, operation: str = "unknown") -> MigrationError:
    ...
    def handle_config_error(self, config_issue: str, config_type: str = "unknown") -> ConfigurationError:
    ...
    def _parse_js_error(self, stderr: str) -> Dict[str, str]:
        ...
        for error_type, pattern in self.js_error_patterns.items():
            ...
            if match:
                ...
                error_info['message'] = match.group(1) if match.groups() else match.group(0)
                break
    ...
    def retry_on_error(
        ...
        def decorator(func: Callable[..., T]) -> Callable[..., T]:
            ...
            def sync_wrapper(*args, **kwargs) -> T:
                return self._execute_with_retry(func, args, kwargs, error_types, exclude_types)
            
            @wraps(func)
            async def async_wrapper(*args, **kwargs) -> T:
                return await self._execute_with_retry_async(func, args, kwargs, error_types, exclude_types)
    ...
    def _execute_with_retry(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                self.logger.debug(f"Not retrying excluded error type: {type(e).__name__}")
                raise
...
def with_error_handling(
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        @retry_handler.retry_on_error(error_types=error_types, exclude_types=exclude_types)
        @wraps(func)
