...
class S3ToStorachaMigrator:
    ...
    async def migrate(
        ...
        """
        logger.info(
            f"Starting migration from {request.source_path} to {request.destination_path}"
        )
        ...
        try:
            ...
            logger.info(
                f"Migration completed successfully: {result.objects_migrated} objects migrated"
            )
            return result
            ...
            duration = (
                time.time() - self._migration_start_time
    ...
    def _prepare_js_input(self, request: MigrationRequest) -> Dict[str, Any]:
        ...
        return {"s3": s3_data, "storacha": storacha_data, "migration": migration_data}

    async def _execute_with_progress_monitoring(
        self, js_input: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        js_task = asyncio.create_task(
            self.js_wrapper.execute_migration(
                js_input["s3"], js_input["storacha"], js_input["migration"]
            )
        )
        ...
        try:
            ...
            if self._cancelled:
                ...
                progress_task.cancel()
                raise MigrationError(
                    "Migration was cancelled by user", operation="migration_cancelled"
                )
        ...
        while not self._cancelled:
            try:
                ...
                estimated_completion = min(
                    elapsed_time / self.migration_config.timeout_seconds, 0.95
                )

                self._progress_reporter.update_progress(
                    current_object="Processing objects... (estimated)",
    ...
    def _parse_js_result(
        self, js_result: Dict[str, Any], request: MigrationRequest
    ) -> MigrationResult:
        ...
        try:
            ...
            result = MigrationResult(
                ...
                if isinstance(failed_objects, list)
                else [],
            )
...
@with_error_handling(
    "simple_migration", error_types=(MigrationError, ConfigurationError, JSWrapperError)
)
async def migrate_s3_to_storacha(
    ...
    **kwargs,
    ...
    request = MigrationRequest(
        source_path=source_path, destination_path=destination_path, **kwargs
    )
