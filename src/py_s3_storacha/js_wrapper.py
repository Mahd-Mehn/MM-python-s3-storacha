...
class JSWrapperManager:
    ...
    async def _execute_js_subprocess(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            stdout_data, stderr_data = await process.communicate(input_json.encode('utf-8'))
            ...
            raise JSWrapperError(
                "JavaScript subprocess execution timed out",
                original_error=e
            )
        except OSError as e:
            raise JSWrapperError(
                f"Failed to execute JavaScript subprocess: {e}",
                original_error=e
            )
    ...
    async def _validate_nodejs(self) -> None:
        ...
        try:
            ...
            raise JSWrapperError(
                f"Failed to execute Node.js version check: {e}",
                original_error=e
            )
    ...
    def _validate_js_script_path(self) -> None:
        ...
        if not script_path.suffix.lower() in ['.js', '.mjs']:
            logger.warning(
