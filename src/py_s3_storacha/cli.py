...
Environment Variables:
  ...
  STORACHA_ENDPOINT_URL   Storacha endpoint URL
        ...
        s3_group.add_argument(
            "--s3-bucket", required=True, help="S3 bucket name (required)"
        )
        s3_group.add_argument(
            "--s3-endpoint-url",
        ...
        storacha_group.add_argument(
            "--storacha-space", required=True, help="Storacha space name (required)"
        )
        ...
        migration_group.add_argument(
            "--dest-path", required=True, help="Destination path in Storacha (required)"
        )
        migration_group.add_argument(
            "--include-pattern",
        ...
        # Version
        parser.add_argument(
            "--version", action="version", version=f"%(prog)s {self._get_version()}"
        )
    ...
    def validate_args(self, args: argparse.Namespace) -> None:
        ...
        if errors:
            raise ConfigurationError(f"Argument validation failed: {'; '.join(errors)}")
...
class CLIConfigurationLoader:
    ...
    def _create_s3_config(self, config_data: Dict[str, Any]) -> S3Config:
        ...
        if not bucket_name:
            raise ConfigurationError("S3 bucket name is required (use --s3-bucket)")
    ...
    def _create_storacha_config(self, config_data: Dict[str, Any]) -> StorachaConfig:
        ...
        # Optional fields with defaults
        endpoint_url = config_data.get(
            "storacha_endpoint_url", "https://api.storacha.network"
        )

        return StorachaConfig(
            api_key=api_key, endpoint_url=endpoint_url, space_name=space_name
        )
    ...
    def create_progress_callback(self):
        ...
        def progress_callback(progress: MigrationProgress) -> None:
            ...
            if self.verbose:
                ...
                print(
                    f"[{progress.current_operation or 'migrating'}] "
                ...
                progress_bar = self._create_progress_bar(
                    progress.progress_percentage, width=40
                )
...
class CLIExecutor:
    ...
    @with_error_handling(
        "cli_execution", error_types=(Exception,), exclude_types=(KeyboardInterrupt,)
    )
    async def execute(self, args: Optional[List[str]] = None) -> int:
        ...
        try:
            ...
            # Load configuration
            s3_config, storacha_config, migration_config = (
                self.config_loader.load_configuration(parsed_args)
            )
            ...
            result = await migrator.migrate(
                request=migration_request, progress_callback=progress_callback
            )
